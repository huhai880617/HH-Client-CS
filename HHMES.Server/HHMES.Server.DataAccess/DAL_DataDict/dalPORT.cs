...
namespace HHMES.Server.DataAccess
{
    ...
    /// dalPORT
    /// </summary>
    public class dalPORT : dalBaseDataDict, IBridge_PORT
    {
         /// <summary>
         /// 构造器
         /// </summary>
         /// <param name="loginer">当前登录用户</param>
         public dalPORT(Loginer loginer): base(loginer)
         {
             _KeyName = tb_PORT.__KeyName; //主键字段
             _TableName = tb_PORT.__TableName;//表名
             _ModelType = typeof(tb_PORT);
         }
         ...
         {
           Type ORM = null;
           if (tableName == tb_PORT.__TableName) ORM = typeof(tb_PORT);
