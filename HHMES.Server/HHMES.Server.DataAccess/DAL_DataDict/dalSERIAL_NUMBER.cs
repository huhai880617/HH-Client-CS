...
namespace HHMES.Server.DataAccess
{
    ...
    /// dalSERIAL_NUMBER
    /// </summary>
    public class dalSERIAL_NUMBER : dalBaseDataDict, IBridge_SERIAL_NUMBER
    {
         /// <summary>
         /// 构造器
         /// </summary>
         /// <param name="loginer">当前登录用户</param>
         public dalSERIAL_NUMBER(Loginer loginer): base(loginer)
         {
             _KeyName = tb_SERIAL_NUMBER.__KeyName; //主键字段
             _TableName = tb_SERIAL_NUMBER.__TableName;//表名
             _ModelType = typeof(tb_SERIAL_NUMBER);
         }
         ...
         {
           Type ORM = null;
           if (tableName == tb_SERIAL_NUMBER.__TableName) ORM = typeof(tb_SERIAL_NUMBER);
