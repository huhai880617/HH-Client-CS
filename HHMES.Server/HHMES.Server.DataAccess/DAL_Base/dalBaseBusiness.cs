...
namespace HHMES.Server.DataAccess.DAL_Base
{
    ...
    public class dalBaseBusiness : dalBase
    {
             ...
             string keyFieldName, ref string GUID, string docNoFieldName, ref string docNo)
        {
            ...
            if (row.RowState == DataRowState.Added)
            {
                ...
                if (model == UpdateKeyMode.OnlyDocumentNo)
                {
                    ...
                    //
                    DocHeaer = GetDocHeader(row["Bill_BillType"].ToString());
        ...
        /// <returns></returns>
        public virtual string GetDocHeader(string DocType)
        {
            string str = "BN";
            switch (DocType.Substring(0, 1))
            {
                case "1":
                    {
                        str = "BI"; break;
