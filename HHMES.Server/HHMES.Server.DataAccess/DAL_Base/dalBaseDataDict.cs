...
namespace HHMES.Server.DataAccess.DAL_Base
{
    ...
    public class dalBaseDataDict : dalBase, IBridge_DataDict
    {
        ...
        public virtual DataTable ExecuteSqlText(string sqlCmd,string tableName)
        {
            return DataProvider.Instance.GetTable(_Loginer.DBName, sqlCmd, tableName);
        }
        /// <summary>
        /// 下载所有数据字典
        /// </summary>
        ...
        /// <summary>
        /// 执行存储过程，返回数据集 Table[0]为结果判断表格 后面的Table是查询数据
        /// </summary>
        /// <param name="sqlcmd"></param>
        /// <returns></returns>
        public virtual DataSet ExecuteProcedure(string sqlcmd)
        {
            return DataProvider.Instance.GetDataSet(_Loginer.DBName, sqlcmd);
        }
        ...
        public static dalBaseDataDict CreateDalByORM(Loginer loginer, string ORM_TypeName)
        {
            ...
            {
                //
                //哈希表不存在,表示第一次加载,需要从HHMES.Server.DataAccess程序集中查询该类                
                //
