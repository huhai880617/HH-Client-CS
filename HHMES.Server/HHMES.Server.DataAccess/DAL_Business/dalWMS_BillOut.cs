...
namespace HHMES.Server.DataAccess
{
    ...
    /// dalWMS_Bill
    /// </summary>
    public class dalWMS_BillOut : dalBaseBusiness,IBridge_WMS_BillOut
    {
         /// <summary>
         /// 构造器
         /// </summary>
         /// <param name="loginer">当前登录用户</param>
         public dalWMS_BillOut(Loginer loginer): base(loginer)
         {
             _SummaryKeyName = tb_WMS_BillOut.__KeyName; //主表的主键字段
             _SummaryTableName = tb_WMS_BillOut.__TableName;//主表表名
         ...
         {
             Type ORM = null;
             if (tableName == tb_WMS_BillOut.__TableName) ORM = typeof(tb_WMS_BillOut);
             if (tableName == tb_WMS_BillOutDtl.__TableName) ORM = typeof(tb_WMS_BillOutDtl);
          ...
          public DataTable GetSummaryByParam(string Condition)
          {
              ...
              if (sql.ToString() != "") //有查询条件
              {
                  ...
                  DataTable dt = DataProvider.Instance.GetTable(_Loginer.DBName, cmd.SqlCommand, tb_WMS_BillOut.__TableName);
                  return dt;
              }
              else //无查询条件不返回数据
              {
          ...
          {
              string sql1 = " select * from [WMS_BillOut]    where ["+tb_WMS_BillOut.__KeyName+"]=@DocNo1 ;";
              string sql2 = " select * from [WMS_BillOutDtl]   where ["+tb_WMS_BillOutDtl.__KeyName+"]=@DocNo2 ORDER BY IsId "; //明细表排序
              ...
              ds.Tables[0].TableName = tb_WMS_BillOut.__TableName;
              ds.Tables[1].TableName =tb_WMS_BillOutDtl.__TableName;//明细表
              return ds;
          }
          ...
          {
              string sql1 = "DELETE [WMS_BillOut]  where ["+tb_WMS_BillOut.__KeyName+"]=@DocNo1 ";
              string sql2 = "DELETE [WMS_BillOutDtl] where ["+tb_WMS_BillOutDtl.__KeyName+"]=@DocNo2 ";
              SqlCommandBase cmd = SqlBuilder.BuildSqlCommandBase(sql1 + sql2);
              cmd.AddParam("@DocNo1", SqlDbType.VarChar, docNo.Trim());
              cmd.AddParam("@DocNo2", SqlDbType.VarChar, docNo.Trim());
