...
namespace HHMES.Server.DataAccess
{
    ...
    /// dalWMS_Bill
    /// </summary>
    public class dalWMS_BillCheck : dalBaseBusiness,IBridge_WMS_BillCheck
    {
         /// <summary>
         /// 构造器
         /// </summary>
         /// <param name="loginer">当前登录用户</param>
         public dalWMS_BillCheck(Loginer loginer): base(loginer)
         {
             _SummaryKeyName = tb_WMS_BillCheck.__KeyName; //主表的主键字段
             _SummaryTableName = tb_WMS_BillCheck.__TableName;//主表表名
         ...
         {
             Type ORM = null;
             if (tableName == tb_WMS_BillCheck.__TableName) ORM = typeof(tb_WMS_BillCheck);
             if (tableName == tb_WMS_BillCheckDtl.__TableName) ORM = typeof(tb_WMS_BillCheckDtl);
          ...
          {
              string sql1 = " select * from [WMS_BillCheck]    where ["+tb_WMS_BillCheck.__KeyName+"]=@DocNo1 ;";
              string sql2 = " select * from [WMS_BillCheckDtl]   where ["+tb_WMS_BillCheckDtl.__KeyName+"]=@DocNo2 ORDER BY IsId "; //明细表排序
              ...
              ds.Tables[0].TableName = tb_WMS_BillCheck.__TableName;
              ds.Tables[1].TableName =tb_WMS_BillCheckDtl.__TableName;//明细表
              return ds;
          }
          ...
          {
              string sql1 = "DELETE [WMS_BillCheck]  where ["+tb_WMS_BillCheck.__KeyName+"]=@DocNo1 ";
              string sql2 = "DELETE [WMS_BillCheckDtl] where ["+tb_WMS_BillCheckDtl.__KeyName+"]=@DocNo2 ";
              SqlCommandBase cmd = SqlBuilder.BuildSqlCommandBase(sql1 + sql2);
              cmd.AddParam("@DocNo1", SqlDbType.VarChar, docNo.Trim());
              cmd.AddParam("@DocNo2", SqlDbType.VarChar, docNo.Trim());
