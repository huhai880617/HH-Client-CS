...
namespace HHMES.Server.DataAccess
{
    ...
    /// dalWMS_Bill
    /// </summary>
    public class dalWMS_BillIn : dalBaseBusiness,IBridge_WMS_BillIn
    {
         /// <summary>
         /// 构造器
         /// </summary>
         /// <param name="loginer">当前登录用户</param>
         public dalWMS_BillIn(Loginer loginer): base(loginer)
         {
             _SummaryKeyName = tb_WMS_BillIn.__KeyName; //主表的主键字段
             _SummaryTableName = tb_WMS_BillIn.__TableName;//主表表名
         ...
         protected override IGenerateSqlCommand CreateSqlGenerator(string tableName)
         {
          ...
          {
              string sql1 = " select * from [WMS_BillIn]    where ["+tb_WMS_BillIn.__KeyName+"]=@DocNo1 ;";
              string sql2 = " select * from [WMS_BillInDtl]   where ["+tb_WMS_BillIn.__KeyName+"]=@DocNo2 ORDER BY IsId "; //明细表排序
              ...
              ds.Tables[0].TableName = tb_WMS_BillIn.__TableName;
              ds.Tables[1].TableName =tb_WMS_BillInDtl.__TableName;//明细表
              return ds;
          }
