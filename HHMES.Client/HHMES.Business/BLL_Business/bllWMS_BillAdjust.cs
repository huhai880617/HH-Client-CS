...
namespace HHMES.Business
{
    ...
    /// bllWMS_Bill
    /// </summary>
    public class bllWMS_BillAdjust : bllBaseBusiness,IFuzzySearchSupportable
    {
        private IBridge_WMS_BillAdjust _Bridge = null;
         /// <summary>
         /// 构造器
         /// </summary>
        public bllWMS_BillAdjust()
         {
             _KeyFieldName = tb_WMS_BillAdjust.__KeyName; //主键字段
             _SummaryTableName = tb_WMS_BillAdjust.__TableName;//表名
             _Bridge = this.CreateBridge();//实例化桥接功能
         }

        private IBridge_WMS_BillAdjust CreateBridge()
        {
            if (BridgeFactory.BridgeType == BridgeType.ADODirect)
                {
                    return new ADODirect_WMS_BillAdjust().GetInstance();
                }
          ...
          {
              DataSet ds = new dalWMS_BillAdjust(Loginer.CurrentUser).GetBusinessByKey(keyValue); 
          ...
          {
              return new dalWMS_BillAdjust(Loginer.CurrentUser).Delete(keyValue);
          }
          ...
          {
              return new dalWMS_BillAdjust(Loginer.CurrentUser).CheckNoExists(keyValue);
          }
          ...
          public override SaveResult Save(DataSet saveData)
          {
              ...
              return new dalWMS_BillAdjust(Loginer.CurrentUser).Update(saveData); //交给数据层处理
          }
          ...
          public override void ApprovalBusiness(DataRow summaryRow)
          {
               ...
               string key = ConvertEx.ToString(summaryRow[tb_WMS_BillAdjust.__KeyName]);
               new dalWMS_BillAdjust(Loginer.CurrentUser).ApprovalBusiness(key, "Y", Loginer.CurrentUser.Account, DateTime.Now);
          }
          ...
          public override void ClearAppInfo(DataRow summaryRow)
          {
              ...
              string key = ConvertEx.ToString(summaryRow[tb_WMS_BillAdjust.__KeyName]);
              new dalWMS_BillAdjust(Loginer.CurrentUser).ClearAppInfo(key);
          }
          /// <summary>
          ///新增一张业务单据
          /// </summary>
          ...
          {
              DataTable summaryTable = _CurrentBusiness.Tables[tb_WMS_BillAdjust.__TableName];
              DataRow row = summaryTable.Rows.Add();
              row[tb_WMS_BillAdjust.__KeyName] = "*自动生成*";
              row[tb_WMS_BillAdjust.VerNo] = "01";
              row[tb_WMS_BillAdjust.CreatedBy] = Loginer.CurrentUser.Account;
              row[tb_WMS_BillAdjust.CreationDate] = DateTime.Now;
              row[tb_WMS_BillAdjust.LastUpdatedBy] = Loginer.CurrentUser.Account;
              row[tb_WMS_BillAdjust.LastUpdateDate] = DateTime.Now;
           }
          ...
          public override DataSet CreateSaveData(DataSet currentBusiness, UpdateType currentType)
          {
              ...
              DataSet save = this.DoCreateTempData(currentBusiness, tb_WMS_BillAdjust.__TableName);
              ...
              DataTable detail = currentBusiness.Tables[tb_WMS_BillAdjustDtl.__TableName].Copy();
          ...
          {
              string key = this.DataBinder.Rows[0][tb_WMS_BillAdjust.__KeyName].ToString();//取单据号码
          ...
          {  //单独处理,即使错误,不向外抛出异常
              try
              {
                  ...
                  logBridge.WriteLog(logID, original.Tables[0], changes.Tables[0], tb_WMS_BillAdjust.__TableName, tb_WMS_BillAdjust.__KeyName, true); //主表
                  logBridge.WriteLog(logID, original.Tables[1], changes.Tables[1], tb_WMS_BillAdjustDtl.__TableName, tb_WMS_BillAdjustDtl.__KeyName, false);//明细
              }
              catch
              {
