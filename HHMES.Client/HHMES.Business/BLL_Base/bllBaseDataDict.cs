...
namespace HHMES.Business.BLL_Base
{
    ...
    public class bllBaseDataDict : bllBase, ILogSupportable
    {
        ...
        /// 标记是否保存日志
        /// </summary>
        protected bool _WriteDataLog = true;
        ...
        /// 执行存储过程 
        /// </summary>
        /// <param name="sqlcmd">exec sp_Name x,x,x..</param>
        /// <returns></returns>
        public DataSet ExecuteProcedure(string sqlcmd)
        {
            return _DataDictBridge.ExecuteProcedure(sqlcmd);
        }

        public DataTable ExecuteSqltext(string sqlCmd, string tableName)
        {
            return _DataDictBridge.ExecuteSqlText(sqlCmd, tableName);
        }
        ...
        {
            return _DataDictBridge.CheckNoExists(TableName,sqlCondition);
        }
        /// <summary>
        /// 设置缺省值
        /// </summary>
