...
{
    public class ADODirect_PORT
    {
        private IBridge_PORT _DAL_Instance = null;//数据层实例

        public ADODirect_PORT()
        {
            _DAL_Instance = new dalPORT(Loginer.CurrentUser);
        }

        public IBridge_PORT GetInstance()
        {
    ...
    public class WebService_PORT : IBridge_PORT
    {
        public WebService_PORT()
        {
        }

        #region IBridge_PORT 成员
