...
namespace HHMES.ORM
{
    ...
    public class GenerateBLL
    {
        ...
        public string GenerateBusinessBLL(Params4BusinessBLL param)
        {
            ...
            builder.AppendLine("    /// </summary>");
            builder.AppendLine("    public class " + param.BLL_Name + " : bllBaseBusiness,IFuzzySearchSupportable " + (param.SupportBusinessLog ? ",IBusinessLogBLL" : "")); //基类,//支持BusinessLog接口
            builder.AppendLine("    {");
            builder.AppendLine("        private IBridge_" + param.ConcretelyName + " _Bridge = null;");
            builder.AppendLine("         /// <summary>");
            ...
            builder.AppendLine("             _KeyFieldName = " + param.ORM_Name + ".__KeyName; //主键字段");
            builder.AppendLine("             _SummaryTableName = " + param.ORM_Name + ".__TableName;//表名");
            builder.AppendLine("             _Bridge = this.CreateBridge();//实例化桥接功能");
            builder.AppendLine("         }");
            builder.AppendLine("");
            builder.AppendLine("        private IBridge_" + param.ConcretelyName + " CreateBridge()");
            builder.AppendLine("        {");
