...
namespace HHMES.Server.DataAccess
{
    ...
    /// dalWMS_Bill
    /// </summary>
    public class dalWMS_Bill : dalBaseBusiness
    {
         /// <summary>
         /// 构造器
         /// </summary>
         /// <param name="loginer">当前登录用户</param>
         public dalWMS_Bill(Loginer loginer): base(loginer)
         {
             _SummaryKeyName = tb_WMS_Bill.__KeyName; //主表的主键字段
             _SummaryTableName = tb_WMS_Bill.__TableName;//主表表名
         ...
         {
             Type ORM = null;
             if (tableName == tb_WMS_Bill.__TableName) ORM = typeof(tb_WMS_Bill);
             //if (tableName == tb_WMS_Bills.__TableName) ORM = typeof(tb_WMS_Bills);
          ...
          public DataTable GetSummaryByParam(string docNoFrom, string docNoTo, DateTime docDateFrom, DateTime docDateTo)
          {
              ...
              if (sql.ToString() != "") //有查询条件
              {
                  ...
                  DataTable dt = DataProvider.Instance.GetTable(_Loginer.DBName,cmd.SqlCommand, tb_WMS_Bill.__TableName);
                  return dt;
              }
              else //无查询条件不返回数据
              {
          ...
          {
              string sql1 = " select * from [tb_WMS_Bill]    where ["+tb_WMS_Bill.__KeyName+"]=@DocNo1 ";
              string sql2 = " select * from [vw_WMS_Bills]   where ["+tb_WMS_Bill.__KeyName+"]=@DocNo2 ORDER BY Queue "; //明细表排序
              ...
              ds.Tables[0].TableName = tb_WMS_Bill.__TableName;
              //ds.Tables[1].TableName =tb_WMS_Bills.__TableName;//明细表
              return ds;
          }
          ...
          {
              //删除单据:只删除明细,主表金额清零!!!
              string sql1 = "UPDATE [tb_WMS_Bill] SET Amount=0 where ["+tb_WMS_Bill.__KeyName+"]=@DocNo1 ";
              string sql2 = "delete [tb_WMS_Bill] where ["+tb_WMS_Bill.__KeyName+"]=@DocNo2 ";
              SqlCommandBase cmd = SqlBuilder.BuildSqlCommandBase(sql1 + sql2);
              cmd.AddParam("@DocNo1", SqlDbType.VarChar, docNo.Trim());
              cmd.AddParam("@DocNo2", SqlDbType.VarChar, docNo.Trim());
          ...
          {
              string docNo = DocNoTool.GetNumber(tran, "WMS_Bill");
              return docNo;
          }
